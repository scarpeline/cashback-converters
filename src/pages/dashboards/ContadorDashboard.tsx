...
const ContadorDashboard = () => {
  ...
  const [accountantVerified, setAccountantVerified] = useState<boolean | null>(
    null,
  );
  ...
  const navigation = useMemo(
    () => [
      ...
      },
      {
        ...
        href: `${basePath}/automacao-fiscal`,
        icon: BarChart3,
      },
        ...
        <main className="flex-1 p-4 lg:p-6 overflow-auto">
          <Routes>
            ...
            <Route
              ...
              element={<AutomacaoFiscalContadorPage />}
            />
            <Route path="empresas" element={<EmpresasPage />} />
            <Route
              path="vinculos"
              element={<AccountingLinksPanel mode="accountant" />}
            />
            <Route
              path="impostos-guias"
              element={<AccountingTaxesPanel mode="accountant" />}
            />
            <Route
              path="mensagens"
              element={<UniversalChatPanel mode="contador" />}
            />
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const uniqueCompanies = new Set(
        companies.data?.map((c: any) => c.entity_user_id) || [],
      ).size;
      const totalEarnings = (fiscal.data || []).reduce(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Gerencie empresas, declarações e pedidos de serviço
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card
          className={
            stats.pendingRequests > 0 ? "border-primary/30 bg-primary/5" : ""
          }
        >
          <CardHeader className="pb-2">
...
const ServicosContabeisPage = () => {
  ...
  const proposeChange = useCallback(async () => {
    ...
    try {
      parsedRf =
        typeof editing.required_fields === "string"
  ...
  const createService = useCallback(async () => {
    if (!user) return;
    if (!editing?.service_type?.trim()) {
      toast.error("Informe o código do serviço (service_type).");
      return;
    }
    if (!editing?.label?.trim()) {
      toast.error("Informe o nome do serviço (label).");
      return;
    }
    let parsedRf: any[] = [];
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Configure preço e dados requerentes. Alterações exigem aprovação do
            Super Admin.
          </p>
        </div>
        <Button
      ...
      {creating && (
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader>
            ...
            <CardDescription>
              Será exibido para usuários/contador somente após aprovação do
              Super Admin.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Sem espaços. Use letras, números e underline.
                </p>
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <Textarea
                  ...
                  value={
                    typeof editing?.required_fields === "string"
                      ? editing.required_fields
                      : JSON.stringify(editing?.required_fields || [], null, 2)
                  }
                  onChange={(e) =>
                    setEditing({ ...editing, required_fields: e.target.value })
                  }
                />
              </div>
            </div>
            <div className="flex gap-2">
              <Button variant="gold" onClick={createService} disabled={saving}>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <p className="font-semibold">
                        {serviceLabels[s.service_type] || s.label}
                      </p>
                      ...
                      <div className="flex flex-col gap-3 w-full md:max-w-md">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground mt-0.5">
                            Array de {`{key, label, required}`}. Use type,
                            placeholder, options se necessário.
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <Button
  ...
  const updateRequest = useCallback(
    async (id: string, status: string) => {
      ...
      let query = supabase
        ...
        .eq("id", id);
      if (shouldClaim) query = query.is("accountant_id", null);
        ...
        prev.map((r) =>
          r.id === id
            ? {
                ...r,
                status,
                accountant_id: shouldClaim ? accountantId : r.accountant_id,
              }
            : r,
        ),
      );
    },
    [accountantId, requests],
  );
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Nenhum pedido de serviço recebido.
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={r.id}>
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-semibold">
                        {(r as any).profiles?.name || "Cliente"}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {(r as any).profiles?.email}
                        {(r as any).profiles?.whatsapp
                          ? ` • ${(r as any).profiles?.whatsapp}`
                          : ""}
                      </p>
                    </div>
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${sc.color}`}
                    >
                      {sc.label}
                    </span>
                  </div>
...
const ContaBancariaPage = () => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select("bank_info, pix_key, cpf_cnpj")
      .eq("user_id", user.id)
      .maybeSingle()
      .then(({ data }) => {
        if (data?.bank_info) {
  ...
  const handleSave = async () => {
    if (!form.cpf_cnpj || !form.pix_key) {
      toast.error("CPF/CNPJ e chave PIX são obrigatórios.");
      return;
    }
    setSaving(true);
    const bankData = {
    ...
    try {
      ...
      toast.success(
        "Conta bancária salva! Carteira será criada automaticamente.",
      );
    }
    setSaving(false);
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <p className="font-medium">{bankInfo.agency || "-"}</p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
                ...
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <p className="font-medium">{form.pix_key || "-"}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        cpf_cnpj: formatCpfCnpjBR(e.target.value),
                      })
                    }
                ...
                <div>
                  ...
                  <Input
                    value={form.pix_key}
                    onChange={(e) =>
                      setForm({ ...form, pix_key: e.target.value })
                    }
...
const AutomacaoFiscalContadorPage = () => {
  const { user } = useAuth();
  const [linkedBarbershops, setLinkedBarbershops] = useState<
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <BarChart3 className="w-5 h-5" /> Automação Fiscal
          </CardTitle>
          <CardDescription>
            Selecione uma empresa vinculada para acessar a automação fiscal
            completa.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {linkedBarbershops.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              Nenhuma empresa vinculada. Crie vínculos primeiro.
            </p>
          ) : (
      ...
      {selectedId && (
        <FiscalAutomationPanel barbershopId={selectedId} mode="accountant" />
      )}
    </div>
  );
};
