...
export function SocialProofManager({ barbershopId, showPageSelector = false }: SocialProofManagerProps) {
  ...
  const handleCreate = async () => {
    ...
    setShowForm(false);
    setForm({ message: "", type: "fake", pages: ["landing"] });
    fetchProofs();
  };
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader>
            <CardTitle>Criar Prova Social</CardTitle>
            <CardDescription>Mensagens aparecem como popups para visitantes</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                className="mt-1"
                placeholder='Ex: "João acabou de agendar um corte!" ou "15 pessoas agendaram hoje"'
                value={form.message}
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                <Button
                  variant={form.type === "fake" ? "gold" : "outline"}
                  size="sm"
                  onClick={() => setForm({ ...form, type: "fake" })}
                >
                  🎭 Fake (Marketing)
                </Button>
                <Button
                  variant={form.type === "real" ? "gold" : "outline"}
                  size="sm"
                  onClick={() => setForm({ ...form, type: "real" })}
                >
                  ✅ Real (Dados reais)
                </Button>
              </div>
            ...
            {showPageSelector && (
              <div>
                <Label>Páginas onde aparecerá</Label>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Button variant="gold" onClick={handleCreate} disabled={saving} className="w-full sm:w-auto">
